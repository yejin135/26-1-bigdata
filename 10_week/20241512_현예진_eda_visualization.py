...
def load_data(file_name):
    ...
    df["is_attack"] = (df["label"].str.lower() == "anomalous").astype(int)
...
selected_file = st.sidebar.selectbox(
    "분석할 데이터를 선택하세요.",
    ["1만 개 데이터 (csic2010_requests.csv)", "6만 개 데이터 (csic_database.csv)"]
)

if "1만 개" in selected_file:
    target_file = "csic2010_requests.csv"
else:
    target_file = "csic_database.csv"

df = load_data(target_file)

st.sidebar.header("📊 데이터 기본 정보")
st.sidebar.metric("전체 HTTP 요청", f"{len(df):,}건")
st.sidebar.metric("정상 (Normal)", f"{(df['label'].str.lower()=='normal').sum():,}건")
st.sidebar.metric("공격 (Anomalous)", f"{(df['label'].str.lower()=='anomalous').sum():,}건")
